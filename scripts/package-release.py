...
def main():
    ...
    for dependency in metadata["packages"]:
        ...
        for notice in notices:
            source = notice
            if dependency["name"] in ("goish", "goish-macros") and notice.name == "LICENSE":
                # The copyright holder uses project attribution for this distribution.
                source = ROOT / "LICENSES" / "Goish-MIT.txt"
            shutil.copy2(source, target / notice.name)
        dependencies.append({
    ...
    if manifest["source_dirty"]:
        raise RuntimeError("Commit tracked changes before packaging a release.")
    for path in stage.rglob("*"):
        if path.is_file():
            check_paths(path.read_bytes(), str(path.relative_to(stage)))
    archive = destination / (name + ".tar.gz")
    timestamp = int(output("git", "show", "-s", "--format=%ct", "HEAD"))
    ...
    with tarfile.open(archive, "w:gz") as bundle:
        bundle.add(stage, arcname=name, filter=public_metadata)
