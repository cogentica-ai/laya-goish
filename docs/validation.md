...
Run on the Vultr host, Linux x86-64, Intel Xeon (Skylake), four logical CPUs.
...
The binary is statically linked and has no dynamic section. Its SHA-256 is recorded in validation/binary-sha256.txt; model checksums are in validation/model-sha256.txt.
